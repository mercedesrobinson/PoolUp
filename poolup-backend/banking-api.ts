...
// Note: Request interface extension is in auth-middleware.ts
...
router.post('/auth/google', async (req: Request, res: Response) => {
  try {
    ...
    let user = await req.db.get('SELECT * FROM users WHERE google_id = ? OR email = ?', [google_id, email]) as User | undefined;
    
    if (!user) {
      const result = await req.db.run(
        'INSERT INTO users (google_id, name, email, profile_image_url, authProvider, created_at) VALUES (?, ?, ?, ?, ?, ?)',
      ...
      await req.db.run('UPDATE users SET stripe_customer_id = ? WHERE id = ?', [customer.id, user.id]);
    }
...
// Get bank accounts
router.get('/plaid/accounts', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ error: 'User not authenticated' });
    ...
    const plaidItem = await req.db.get('SELECT * FROM plaid_items WHERE user_id = ?', [userId]);
    if (!plaidItem) {
      res.json({ accounts: [] });
      return;
    }
...
// Get account balance
router.get('/plaid/balance/:accountId', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ error: 'User not authenticated' });
    ...
    const plaidItem = await req.db.get('SELECT * FROM plaid_items WHERE user_id = ?', [userId]);
    if (!plaidItem) {
      res.status(404).json({ error: 'No bank account connected' });
      return;
    }
...
router.post('/stripe/create-card', async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ error: 'User not authenticated' });
    ...
    const user = await req.db.get('SELECT * FROM users WHERE id = ?', [userId]) as User & { stripe_customer_id?: string };
...
router.post('/payments/contribute', async (req: Request, res: Response) => {
  try {
...
router.post('/payments/auto-contribute', async (req: Request, res: Response) => {
  try {
...
router.post('/stripe/toggle-card', async (req: Request, res: Response) => {
  try {
    ...
    const card = await req.db.get('SELECT * FROM virtual_cards WHERE stripe_card_id = ? AND user_id = ?', [card_id, userId]);
    if (!card) {
    ...
    await req.db.run('UPDATE virtual_cards SET status = ? WHERE stripe_card_id = ?', [updatedCard.status, card_id]);
...
router.post('/analytics/spending', async (req: Request, res: Response) => {
  try {
    ...
    const startDate = new Date(Date.now() - (parseInt(period || '30') * 24 * 60 * 60 * 1000)).toISOString().split('T')[0];
