...
import { PREMIUM_TIERS, calculateOptimalContribution, getPremiumUpsells, calculateMonthlyRevenue } from './premium';
...
interface PoolMembership {
  ...
  user_id: number;
  joined_at: string;
  penalty_agreed: boolean;
  penalty_agreed_at?: string;
}
...
interface SoloGoalPrivacy {
  id: number;
  user_id: number;
  pool_id: number;
  show_goal_amount: boolean;
  show_current_amount: boolean;
  show_goal_purpose: boolean;
...
app.post('/api/pools', async (req: Request, res: Response) => {
  try {
    ...
    const { userId, name, goalCents, destination, tripDate, poolType, penalty }: CreatePoolRequest = req.body;
    ...
    if (goalCents === undefined || goalCents === null || (typeof goalCents === 'string' && goalCents === '')) {
      finalGoalCents = 0;
    }
    ...
    res.status(500).json({ error: 'Failed to create pool', details: (error as Error).message });
  }
});

// Get user pools
app.get('/api/users/:userId/pools', async (req: Request, res: Response) => {
  try {
...
app.post('/api/users/:userId/payment-methods/link', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
app.get('/api/users/:userId/streak', async (req: Request, res: Response) => {
  try {
    ...
    if (contributions.length > 0) {
      ...
      const daysDiff = Math.floor((today.getTime() - lastContribution.getTime()) / (1000 * 60 * 60 * 24));
      ...
      if (daysDiff <= 7) {
        ...
        for (let i = 1; i < contributions.length; i++) {
          ...
          const weekDiff = Math.floor((prevDate.getTime() - currDate.getTime()) / (1000 * 60 * 60 * 24 * 7));
...
app.post('/api/pools/:poolId/withdraw', async (req: Request, res: Response) => {
  try {
...
app.get('/api/pools/:poolId/penalty-status', async (req: Request, res: Response) => {
  try {
    const { poolId } = req.params;
...
// Premium endpoints
app.get('/api/premium/tiers', (req: Request, res: Response) => {
  res.json(PREMIUM_TIERS);
});

app.get('/api/users/:userId/premium-upsells', async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    
    // Get user's current tier and usage stats
    const user = await db.get('SELECT * FROM users WHERE id = ?', [userId]) as User;
    const poolsCreated = await db.get('SELECT COUNT(*) as count FROM pools WHERE creator_id = ?', [userId]) as { count: number };
    const contributions = await db.all('SELECT * FROM contributions WHERE user_id = ?', [userId]) as Contribution[];
...
app.post('/api/users/:userId/upgrade', async (req: Request, res: Response) => {
  try {
    const { tier } = req.body;
...
app.get('/api/users/:userId/peer-transfers', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
app.get('/api/users/:userId/solo-goals/:poolId/privacy', async (req: Request, res: Response) => {
  try {
    ...
    `, [userId, poolId]) as SoloGoalPrivacy;
...
app.put('/api/users/:userId/solo-goals/:poolId/privacy', async (req: Request, res: Response) => {
  try {
    const { userId, poolId } = req.params;
...
app.get('/api/admin/revenue', async (req: Request, res: Response) => {
  try {
    ...
    const revenue = calculateMonthlyRevenue({ FREE: users.length });
    res.json(revenue);
  } catch (error) {
