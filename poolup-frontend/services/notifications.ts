...
class NotificationService {
  ...
  async schedulePaydayReminders(userId: string, paydaySettings: PaydaySettings): Promise<void> {
    try {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (reminderDate > now) {
          await Notifications.scheduleNotificationAsync({
            ...
            trigger: { type: 'date' as any, date: nextPayday },
          });
        }
      }
  ...
  async scheduleStreakWarning(userId: string, streakData: StreakData): Promise<void> {
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        ...
        trigger: { type: 'date' as any, date: warningDate },
      });
    } catch (error) {
  ...
  async scheduleGoalMilestone(poolId: string, milestone: number): Promise<void> {
    try {
      await Notifications.scheduleNotificationAsync({
        content: {
  ...
  async schedulePeerBoost(fromUser: User, toUser: User, amount: number): Promise<void> {
    try {
      await Notifications.scheduleNotificationAsync({
        content: {
  ...
  async scheduleDailyMotivation(userId: string, enabled: boolean = true): Promise<void> {
    try {
      ...
      for (let i = 1; i <= 7; i++) {
        ...
        const motivationTime = notificationDate;
        ...
        await Notifications.scheduleNotificationAsync({
          ...
          trigger: { type: 'date' as any, date: motivationTime },
        });
      }
    } catch (error) {
  ...
  // Utility functions
  calculateNextPayday(paydaySettings: PaydaySettings, offset: number = 0): Date {
