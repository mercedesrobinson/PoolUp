...
export default function CreatePool({ navigation, route }){
  ...
  const create = async ()=>{
    ...
    const goalCents = Math.round(parseFloat(goalCents) * 100);
    if(goalCents <= 0) return Alert.alert('Error','Valid goal amount required');
    
    await api.createPool(user.id, name.trim(), goalCents, destination.trim(), tripDate, poolType);
    ...
    <ScrollView style={{ flex:1, backgroundColor:'#FAFCFF' }}>
      <View style={{ padding: 24 }}>
        ...
        <TouchableOpacity onPress={create} style={{ backgroundColor: colors.purple, padding:16, borderRadius:radius, alignItems:'center' }}>
          <Text style={{ color:'white', fontSize:18, fontWeight:'700' }}>Create Pool</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
